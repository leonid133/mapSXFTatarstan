...
string codepage::ansi_to_utf8(const string ansi)
{
    ...
    if(! is_ansi_to_local_init)
    {
        ...
        if((desc_ansi_to_local = my_iconv_open("UTF-8", "CP1251")) == (my_iconv_t) -1)
    ...
    my_iconv(desc_ansi_to_local, & p_in_buf, & in_bytes, & p_out_buf, & out_bytes) == (size_t) -1;
