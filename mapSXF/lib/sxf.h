    ...
    */
    ...
    class CSXFMap : public CMap
    {
        struct s_map
        {
            ...
            uint8_t * read_pnt(std::vector<vektor> & pnt, const SRecord & desc, uint8_t * buf, const unsigned pnt_num, const unsigned dim, const bool is_float, const bool is_large);
            //uint8_t * read_label(std::string & label, uint8_t * buf);
            void read_next_obj(CFile & fl);
            vektor cmp_rectangle(const bool is_min) const;
            vektor cmp_frame(const bool is_min) const;
            void unpack_header();
        ...
        struct s_height_map
        {
            ...
            SMTRHeader header;
            std::vector< std::vector<vektor> > pnts;

            void unpack_header();
            void create_point_grid(const std::vector< std::vector<vektor> > & raw_pnt, const vektor from, const vektor to, const unsigned size);
            vektor left_top();
            vektor size_in_map_coord();

            std::vector< std::vector<vektor> > load(const std::string fname);
            void check();
